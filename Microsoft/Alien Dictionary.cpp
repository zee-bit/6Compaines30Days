...
void dfs(int node, vector<vector<int>> &adj, string &ans, vector<bool> &vis) {
    vis[node] = true;
...
string findOrder(vector<string> &dict, int N, int K) {
    ...
    for(int i = 0; i < N-1; i++) {
        ...
        int idx = 0, len = min(a.length(), b.length());
        while(idx < len) {
            if(a[idx] != b[idx]) {
                int i1 = a[idx] - 'a', i2 = b[idx] - 'a';
                adj[i1].push_back(i2);
                break;
            }
            idx++;
