...
// Efficient method without sorting
void solve1() {
	int n;
	cin >> n;
	vector<char> nuts(n), bolts(n);
	rep(i, 0, n) cin >> nuts[i];
	rep(i, 0, n) cin >> bolts[i];
...
void solve2() {
	...
	unordered_map<char, int> mp = {{'!', 1}, {'#', 2}, {'$', 3}, {'%', 4}, {'&', 5}, {'*', 6}, {'@', 7}, {'^', 8}, {'~', 9}};
    sort(all(nuts) [&](char a, char b) {
        return mp[a] < mp[b];
    });
    sort(all(bolts), [&](char a, char b) {
        return mp[a] < mp[b];
    });
}
