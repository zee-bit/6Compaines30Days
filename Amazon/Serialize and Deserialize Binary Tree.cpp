...
void serializeHelper(Node *node, vector<int> &ser) {
    if(node == NULL) {
        ser.push_back(-1);
        return;
    }
    ser.push_back(node -> data);
    serializeHelper(node -> left, ser);
    serializeHelper(node -> right, ser);
}
...
Node *deserializeHelper(vector<int> &A, int &idx) {
    if(idx == A.size() || A[idx] == -1) return NULL;

    Node *root = new Node(A[idx]);
    idx++;
    root -> left = deserializeHelper(A, idx);
    idx++;
    root -> right = deserializeHelper(A, idx);
    return root;
}
...
Node * deSerialize(vector<int> &A)
{
   //Your code here
   int idx = 0;
   return deserializeHelper(A, idx);
}

Node* solve() {
	Node *head;
	// 1. Construct tree
	auto A = serialize(head);
	// 2. Delete tree
	Node *newRoot = deSerialize(A);
	return newRoot;
}
