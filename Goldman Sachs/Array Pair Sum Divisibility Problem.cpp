/*
	PROBLEM STATEMENT: Given an array of integers and a number k, write a function that returns
...
bool solve() {
    ...
    for(int num : nums) {
        int mod = num % k;
        if(mod == 0 && (mod_cnt[mod] & 1))
            return false;
        else if(mod != 0 && mod_cnt[mod] != mod_cnt[k - mod])
            return false;
    }
    return true;
}
