/*
	PROBLEM STATEMENT: For two strings s and t, we say "t divides s" iff s = t + ... + t
...
string gcd2(string s1, string s2) {
	if(s1 + s2 != s2 + s1)
        ...
        int len_gcd = __gcd(s1.length(), s2.length());
        return gcd2(s1.substr(0, len_gcd), s2.substr(0, len_gcd));
    }
    return "";
}
